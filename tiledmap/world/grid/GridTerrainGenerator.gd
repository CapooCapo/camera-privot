...
# ---------------------------------------------------------------------------
# Config — giữ @export để chỉnh từ Inspector
# ---------------------------------------------------------------------------
...
# Kéo thả node TreeSpawner từ scene vào đây trong Inspector
# Node đó phải có tree_scene được gán sẵn
@export var tree_spawner_node: NodePath
...
func generate(grid_map: GridMap) -> void:
...
func update_stream(grid_map: GridMap, delta: float, force := false) -> void:
	_streamer.update_stream(grid_map, delta, force)


func set_runtime_cell(grid_map: GridMap, cell: Vector3i, item_id: int) -> void:
	_loader.set_runtime_cell(grid_map, cell, item_id)
...
func get_surface_cell(world_x: int, world_z: int) -> Vector3i:
	return _loader.get_surface_cell(world_x, world_z)
...
func _wire_modules() -> void:
	...
	_profile.setup(_cfg, _noise)

	_sampler = TerrainSampler.new()
	_sampler.setup(_cfg, _noise, _profile)
	...
	print("tree_spawner_node =", tree_spawner_node)

	if tree_spawner_node.is_empty():
		push_error("TreeSpawner NodePath đang để trống!")
	else:
		var node := get_node_or_null(tree_spawner_node)

		print("Resolved =", node)

		if node == null:
			push_error("Không tìm thấy node: " + str(tree_spawner_node))
		elif node is TreeSpawner:
			_tree_spawner = node
			print("TreeSpawner OK")
		else:
			push_error("Node tìm được không phải TreeSpawner: " + str(node))
	...
	_loader.setup(_cfg, _sampler, _tree_spawner)

	_overlay = BlockGridOverlay.new()
	_overlay.setup(_cfg)

	_streamer = ChunkStreamer.new()
	_streamer.setup(_cfg, _loader, _overlay, self)
